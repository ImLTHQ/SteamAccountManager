...
class ExportDialog(simpledialog.Dialog):
    """用于选择导出方式的对话框（导出TXT或导出到剪贴板）"""
    def __init__(self, parent, title, selected_accounts):
        self.selected_accounts = selected_accounts  # 接收选中的账号数据
        self.export_type = None  # 用于存储用户选择的导出类型
        super().__init__(parent, title)
    ...
    def body(self, master):
        ...
        ttk.Label(
            master, 
            text=lang['select_export_method'],
            wraplength=300
        ...
        # 导出TXT按钮
        self.txt_btn = ttk.Button(
            btn_frame,
            text=lang['txt'],
            width=15,
            command=lambda: self.set_export_type("txt")
        )
        self.txt_btn.pack(side=tk.LEFT, padx=10)
        ...
        self.clipboard_btn = ttk.Button(
            ...
            text=lang['clipboard'],
            width=15,
            command=lambda: self.set_export_type("clipboard")
        )
    ...
    def apply(self):
        ...
        if not self.export_type:
            ...
            self.result = (self.export_type, self.selected_accounts)

    def buttonbox(self):
        """重写按钮盒方法，不显示默认按钮（我们使用自定义按钮）"""
        pass